...
public class DefaultDragHelper implements DragHelper {
    ...
    public DefaultDragHelper(@NonNull List<WXComponent> dataSource, @NonNull RecyclerView
            recyclerView, @NonNull EventTrigger trigger) {
    ...
    public void onDragging(int fromPos, int toPos) {
        ...
        if (fromPos >= 0 && fromPos <= mDataSource.size() - 1 && toPos >= 0 && toPos <=
                mDataSource.size() - 1) {
    ...
    public boolean isDragExcluded(@NonNull RecyclerView.ViewHolder viewHolder) {
        ...
        return viewHolder.itemView.getTag() != null && TAG_EXCLUDED.equals(viewHolder.itemView
                .getTag());
    }

    @Override
    public void setVibrate(boolean mDragVibrate) {
        isVibrate = mDragVibrate;
    }
